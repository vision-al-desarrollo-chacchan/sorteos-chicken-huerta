...
export default function Home() {
  ...
  async function registrar(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/participantes", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ nombre: form.get("nombre"), dni: form.get("dni"), celular: form.get("celular"), cantidad, operacion: form.get("operacion") }) });
  ...
  return <main>
    <header className="nav"><a className="brand" href="#inicio"><span>CH</span> Chicken Huerta</a><nav><a href="#premios">Premios</a><a href="#participar">Participar</a><a href="#consultar">Consultar ticket</a></nav></header>
    ...
    <div className="heroCard" id="premios"><p>Participas automáticamente por</p><div className="prize first"><span>1.er premio</span><strong>2 pollos enteros</strong><small>Pollo a la brasa + papas + ensalada</small></div><div className="prize"><span>2.do premio</span><strong>1 pollo entero</strong><small>Pollo a la brasa + papas + ensalada</small></div><div className="secure">✓ Tickets únicos y verificables</div></div></section>
